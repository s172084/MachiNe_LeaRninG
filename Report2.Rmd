...
The measurements of diamond length, width and depth were converted from millimetres (mm) to micrometers (µm). Carat was converted to milligrams (mg) and the outliers, which are the values that lie outside of the upper boundary and lower boundaries, were removed. The *total depth percentage* is called *depth* and it represents: $$\frac{depth} {mean( length * width)}$$
...
**Are some of the models identical? ****What recommendations would you make based on what you've learned? **

The paired t-test is used to determine if the mean difference between two sets of observations is zero. The differences have a Normal  Distribution. The Null Hypothesis $H_{0}$ is that the mean of the differences is 0, whereas the alternative hypotheses $H_{1}$ is that the mean of the differences is not 0. The p-value indicates whether the difference in mean squared errors between the models is statistically significant.

Evaluation of the values from the Artificial Neural Network against the values from the Baseline show that the p-value is less than 2.2e-16 and the confidence interval is  - 3009.865 up to - 3009.776 . This indicates that the null hypothesis can be rejected. 

Evaluation of the values from the Regularised Linear Regression against values from the Baseline, 
Show that the p-value is 1 and the confidence interval is - 166 up to 166 . This indicates that the null hypothesis can be accepted. 

Evaluation of the values from the Artificial Neural Network against the Regularised Linear Regression, show that p-value is 2.2e-16 and the confidence interval is - 3176.502 up to - 2843.138. This indicates that the null hypothesis can be rejected.
...
```{r echo=FALSE,   fig.alt='left', fig.show='asis', out.width='55%', fig.cap= "Figures B and C"}
...
```{r echo=FALSE, fig.align='center', out.width='80%', fig.cap="Figure D" }
# Include images and pictures..
